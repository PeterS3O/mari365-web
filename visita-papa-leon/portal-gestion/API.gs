...
function columnasInscriptos() {
  ...
  const cols = {
    ...
    emailPortal: firstIndex([
      idxExact(headers, 'Email'),
      idxByTokens(headers, ['email'], ['portal', 'pagos']),
      idxByTokens(headers, ['email']),
    ]),
    celular: idxByTokens(headers, ['celular']),
    emergencia: firstIndex([
      idxExact(headers, 'Telefono emergencia'),
      idxByTokens(headers, ['emergencia']),
    ]),
